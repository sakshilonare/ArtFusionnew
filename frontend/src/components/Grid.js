...
const Grid = ({ photos }) => {
  ...
  const saveChanges = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/update/${selectedPhoto._id}`, {
        method: 'PUT',
        headers: {
  ...
  const deletePic = async (photoId) => {
    try {
      const response = await fetch(`http://localhost:5000/api/delete/${photoId}`, {
        method: 'DELETE',
      });
  ...
  function MyVerticallyCenteredModal(props) {
    ...
    const saveImageToDrive = async (photoId) => {
      try {
        const response = await fetch(`http://localhost:5000/api/save-to-drive/${photoId}`, {
          method: 'POST',
        });
